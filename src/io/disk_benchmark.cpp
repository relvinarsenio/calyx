...
public:
    ...
    [[nodiscard]] std::expected<void, posix::SysCallError> finalize_write() noexcept {
        if (const auto sync { file_.datasync() }; !sync) {
            return std::unexpected { posix::SysCallError { sync.error(), "fdatasync" } };
        }
        if (const auto adv { file_.advise(0, 0, posix::FAdvise::DontNeed) }; !adv) {
...
/**
 * @brief Populates a memory buffer with deterministic pseudo-random bytes using 16 interleaved PRNG streams.
 * @details Uses compile-time Galois Field GF(2)[x] polynomial jumping to partition 16 independent
 * Xoshiro256++ generators spaced 2^128 steps apart. Interleaves 8-byte outputs across a 128-byte block
 * to eliminate data dependencies and maximize superscalar CPU pipeline throughput.
 */
void fill_pattern_fast(std::span<std::byte> buffer) noexcept {
    static constexpr auto kStreams { 16uz };
    static constexpr auto kWordBits { std::countr_zero(sizeof(std::uint64_t)) };
    static constexpr auto kBlockBytes { kStreams << kWordBits };

    static constexpr auto kInitialEngines { []<std::size_t... Is>(std::index_sequence<Is...>) consteval {
        prng::Xoshiro256PlusPlus master { 0x243f6a8885a308d3ULL };
        const auto next_stream { [&master](const auto) constexpr noexcept {
            const auto current { master };
            master.jump();
            return current;
        } };
        return std::array<prng::Xoshiro256PlusPlus, sizeof...(Is)> { next_stream(Is)... };
    }(std::make_index_sequence<kStreams> {}) };

    auto engines { kInitialEngines };

    std::ranges::for_each(buffer | std::views::chunk(kBlockBytes), [&engines](auto chunk) noexcept {
        std::array<std::uint64_t, kStreams> words {};
        std::ranges::transform(engines, words.begin(), [](auto& eng) noexcept { return eng(); });
        const auto bytes { std::bit_cast<std::array<std::byte, kBlockBytes>>(words) };
...
[[nodiscard]] std::expected<std::filesystem::path, BenchmarkError> perform_space_check(
    ...
    if (ec) {
        return std::unexpected { BenchmarkError {
            BenchmarkError::Phase::SpaceCheck, posix::SysCallError { ec, "path resolution" } } };
    }

    if (const auto res { check_disk_space(target_path, total_bytes) }; !res) {
        return std::unexpected { BenchmarkError {
            BenchmarkError::Phase::SpaceCheck, posix::SysCallError { res.error(), "Storage" } } };
    }
    return target_path;
}
...
/**
 * @brief Validates pre-conditions on user benchmark configurations.
 */
[[nodiscard]] constexpr std::expected<void, BenchmarkError> validate_config(
    const DiskBenchmark::BenchmarkConfig& config) noexcept {
...
/**
 * @brief Encapsulates post-alignment I/O geometry computed from configuration and hardware constraints.
 */
struct IoGeometry {
...
/**
 * @brief Computes safe, aligned I/O geometry satisfying O_DIRECT kernel and hardware constraints.
 */
[[nodiscard]] std::expected<IoGeometry, BenchmarkError> compute_io_geometry(
    const DiskBenchmark::BenchmarkConfig& config, const posix::BlockSize& hw) noexcept {
    /**
    ...
    const auto write_block_size { *write_block_size_opt };
    ...
    const auto read_block_size { *read_block_size_opt };
    ...
    const auto total_bytes_opt { round_up(raw_size, *final_mask_res) };
    if (!total_bytes_opt) {
        return std::unexpected { BenchmarkError { BenchmarkError::Phase::BufferAllocation,
 ...
 */
[[nodiscard]] std::expected<Buffers, BenchmarkError> allocate_io_buffers(
    const IoGeometry& geom, const DiskBenchmark::BenchmarkConfig& config) {
    const auto write_buf_total { safe_mul(geom.write_mem_stride, config.write_queue_depth) };
    if (!write_buf_total) {
        return std::unexpected { BenchmarkError { BenchmarkError::Phase::BufferAllocation,
    ...
    auto write_buf_opt { memory::AlignedBuffer::create(write_buf_alloc, geom.alignment) };
    if (!write_buf_opt) {
        return std::unexpected { BenchmarkError {
    ...
    const auto read_buf_alloc_opt { round_up(geom.read_block_size, get_page_size()) };
    ...
    auto read_buffers_res { std::ranges::fold_left(std::views::iota(0uz, toSize(config.read_queue_depth)),
        ...
        [read_buf_alloc, alignment { geom.alignment }](
...
[[nodiscard]] std::expected<PhaseRunStats, BenchmarkError> run_write_operations(
    ...
    const auto io_stats { engine.execute_write(ctx).transform_error(
        [](const uring::UringError& err) { return BenchmarkError { BenchmarkError::Phase::WritePhase, err }; }) };

    if (!io_stats) { return io_stats; }

    if (const auto fin { wf.finalize_write() }; !fin) {
        return std::unexpected { BenchmarkError { BenchmarkError::Phase::WritePhase, fin.error() } };
    }
...
[[nodiscard]] std::expected<PhaseRunStats, BenchmarkError> run_read_operations(
    ...
    return engine.execute_read(ctx).transform_error(
...
[[nodiscard]] std::expected<DiskIORunResult, BenchmarkError> DiskBenchmark::run_io_test(const BenchmarkConfig& config,
    ...
    if (const auto val { validate_config(config) }; !val) { return std::unexpected { val.error() }; }
    ...
    const auto geom_res { compute_io_geometry(config, hw) };
    if (!geom_res) { return std::unexpected { geom_res.error() }; }
    const auto& geom { *geom_res };
    ...
    const auto space_check_res { perform_space_check(geom.total_bytes, dir_path) };
    if (!space_check_res) { return std::unexpected { space_check_res.error() }; }

    auto buffers_res { allocate_io_buffers(geom, config) };
    ...
    const IoParams params {
        ...
        .write_block_size  = geom.write_block_size,
        .read_block_size   = geom.read_block_size,
        .write_mem_stride  = geom.write_mem_stride,
        .read_mem_stride   = geom.read_mem_stride,
        .label             = config.label,
        .write_queue_depth = config.write_queue_depth,
        .read_queue_depth  = config.read_queue_depth,
    };

    const auto write_stats { execute_write_phase(filename, engine, params) };
    if (!write_stats) { return std::unexpected { write_stats.error() }; }

    const auto read_stats { execute_read_phase(filename, engine, params) };
    if (!read_stats) { return std::unexpected { read_stats.error() }; }

    return DiskIORunResult {
        .label = config.label,
        .write = make_disk_metrics(*write_stats),
        .read  = make_disk_metrics(*read_stats),
    };
}
